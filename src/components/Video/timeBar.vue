...
export default {
  ...
  mounted: function() {
    ...
    const { canvas, ctx } = this.initCanvas("vehicleTimeScale", option);
    this.canvas = canvas;
    this.ctx = ctx;
    // canvas时间轴加鼠标点击按下事件
  ...
  methods: {
    ...
    getTimeLineXpoint(canvas, ctx) {
      ...
      const perStep = canvas.width / (288 + 2);
    ...
    drawTimeLineAText(height) {
      ...
      for (let i = 0; i < this.scaleXpointArr.length; i++) {
        if (i % 2 === 0) {
          ...
          let textContent1 = ":" + (i % 12) * 5;
          if (i % 12 == 0) {
            textContent1 = textContent1 + "0";
          }
          this.drawText2d(
            this.ctx,
            textContent1,
    ...
    currentTimeSet(time) {
      ...
      let length = this.perSecondStep * 288 + 2;
      let offsetX2 =
    ...
    getVehiclePosition(vehicleOrderStatus) {
      ...
      // console.log(101011, this.vehicleStatusCanvas);
      this.drawVehicleOrderStatus();
    },

    // 计算开始时间结束时间对应的x轴位置
    getXposition(vehicleOrderStatus) {
      // console.log('时间', this.scaleTimePosition);
      ...
      const startX = startTimeDiff
        ? (startTimeDiff / 300000) * this.perSecondStep +
          this.scaleTimePosition.positionX
        : this.scaleTimePosition.positionX;
      const endX = endTimeDiff
        ? (endTimeDiff / 300000) * this.perSecondStep +
          this.scaleTimePosition.positionX
        : this.scaleTimePosition.positionX;
    ...
    onMouseup(event) {
      if (this.isClick) {
        const clicksetX = event.offsetX + 1;
    ...
    getIsExit2(x) {
      // console.log(22222, x);
      // console.log(22223, this.vehicleStatusCanvas);
      // console.log(22224, this.perSecondStep);
      let timeX = x - this.perSecondStep;
      let length = this.perSecondStep * 288 + 2;
      let zeroTime = new Date(
        new Date(this.dateTime).toLocaleDateString()
      ).getTime();
      let ms = zeroTime + parseInt((timeX * 1000 * 60 * 60 * 24) / length);
      return ms;
      // if (
