...
<template>
  <div class="time-scale-container">
    ...
    <canvas id="vehicleTimeScale3" class="time-scale-canvas"></canvas>
  </div>
</template>
...
export default {
  ...
  mounted: function() {
    ...
    const { canvas, ctx } = this.initCanvas("vehicleTimeScale3", option);
    this.canvas = canvas;
    this.ctx = ctx;
    // canvas时间轴加鼠标点击按下事件
  ...
  methods: {
    ...
    getTimeLineXpoint(canvas, ctx) {
      ...
      const perStep = canvas.width / (24 + 2);
    ...
    currentTimeSet(time) {
      ...
      let length = this.perSecondStep * 24 + 2;
      let offsetX2 =
    ...
    getXposition(vehicleOrderStatus) {
      ...
      const startX = startTimeDiff
        ? (startTimeDiff / 3600000) * this.perSecondStep +
          this.scaleTimePosition.positionX
        : this.scaleTimePosition.positionX;
      const endX = endTimeDiff
        ? (endTimeDiff / 3600000) * this.perSecondStep +
          this.scaleTimePosition.positionX
        : this.scaleTimePosition.positionX;
    ...
    handler(event) {
      ...
      const ele = document.querySelector(".time-scale-tooltip");
